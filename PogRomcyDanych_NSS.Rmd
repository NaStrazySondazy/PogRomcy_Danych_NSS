...
Omówiliśmy już wstępnie dobór losowy respondentów do badania sondażowego. Wiemy, że wyniki z prób przeciętnie powinny "trafiać w punkt". Pozostaje jednak problem błędów. Jak zwuważyliśy na przykładzie sondażu dotyczącego absencji, duża część prób może się myslić. Cóz z tego więc, że nasza metoda doboru respondentów przeciętnie daje dobre wyniki skoro my wylosujemy tylko jedną próbę i ona właśnie chybi. Schemat doboru próby musi więc dostarczać oszacowania, które znajdą się przynajmniej w pobliżu rzeczywistych wartości. Innymi słowy, musi gwarantować, że dla przeważającej większości prób wyniki będa poprawne bądź bardzo zbliżone do poprawnych. Ryzyko popełnienia błędu powinno być jak najmniejsze.       
Zagadnienie błędu oszacowania omówimy na nowym przykładzie. Znajmiemy się poparciem dla partii X w naszej przykadowej populacji.    
...
```
### Tabela 8.1. Liczba prób, w których występują poszczególni obywatele
```{r, echo=FALSE, results='asis'}
p6.1<-NULL
for(i in 1:20) {
...
Poniżej znajduje się ilustracja na pokazany jest efekt zwiększania próby od 6 do 19 respondentów. J
...
__Zadania 7:__   
1. Jaką próbę możemy nazwać reprezentatywną?
...
```{r, echo=FALSE }
p6.5<-data.frame(table(rowSums(p6.5=="W")));
p6.5<-data.frame(p6.5, round(prop.table( p6.5$Freq)*100));
colnames(p6.5)<c("Liczba dostępnych respondentów", "Liczba prób", "Odsetek prób");
kable(x =  p6.5, format = "markdown", row.names = F);
## przerobic na tabele
# ggplot(p6.5, aes(x = factor(Var1), y = Freq)) + 
# geom_bar(stat="identity")+
#   # scale_x_discrete(labels=c("0","1/6","2/6","3/6", "4/6", "5/6", "1"))+
#   theme(axis.text.x = element_text(face="bold", color = "black", size=15))+
#   xlab("Poparcie w próbie")+
#   ylab("Liczba prób")+
#    geom_text(data=p6.5, aes(x = factor(Var1), y = Freq, label=Freq) ,vjust=0.2, size=4, face="bold")
```

Jak widać tylko dla znikomej częśći prób liczba dostępnych respondnetów wyniesie 6. Większość będzie zawierała nie więcej niż 3. W tej systuacji oczywiste jest, że rozkład wyników z prób będzie inny niż poprzednio, gdy dostępni byli wszyscy respondenci. Duże znaczenie mają próby, w których dostępnych jest tylko jeden respondnet lub żaden. Dla takich przypadków nie potrafimy wyznaczyć poziomu poparcia dla partii X (dla prób jednoosobowych jest to możliwe - 0% lub 100% - ale nie możemy w takim przypadku szcować zmienności populacji w próbie więc zostawimy ten przykład). 
