...
Omówiliśmy już wstępnie dobór losowy respondentów do badania sondażowego. Wiemy, że wyniki z prób przeciętnie powinny "trafiać w punkt". Pozostaje jednak problem błędów. Jak zwuważyliśy na przykładzie sondażu dotyczącego absencji, duża część prób może się myslić. Cóz z tego więc, że nasza metoda doboru respondentów przeciętnie daje dobre wyniki skoro my wylosujemy tylko jedną próbę i ona właśnie chybi. Schemat doboru próby musi więc gwarantować, że nasze oszacowania znajdą się przynajmniej w pobliżu rzeczywistych wartości.Innymi słowy, że dla przeważającej większości prób wyniki będa poprawne bądź bardzo zbliżone do poprawnych, a dla pozostałych również nie bardzo odległe.       
Rozwiązanie problemu błędów jakie popełnia się korzystając z prób omówimy na nowym przykładzie. Tym razem znajmiemy się poparciem dla partii X w naszej przykadowej populacji.    

### Tabela 8.1. Rozkład miejsca zamieszkania i preferencji wyborczych w populacji
```{r,echo=FALSE, results='asis'}
kable( poparcie_dla_P, format = "markdown");
```
W stosunku do badania absencji wyborczej zmienimy niewiele. Po prostu zamiast wartości -1 w kolumnie “Glosowanie” teraz będziemy analizowali występowanie wartości 1, czyli glosowanie na partię X. Będziemy również dobierać nie 2, a 6 obywateli do próby. Tak samo jak wcześniej zastosujemy jednak losowanie proste bez zwracania i technikę CAPI do realizacji wywiadów. Pytanie w kwestionariuszu będzie brzmiało: "Gdyby wybory do parlamentu (sejmu i senatu) odbywały się w najbliższą niedzielę to zagłosowałaby/łby Pani/an na partię X"    

Zacznijmy od sprawdzenia, jakie poparcie dla partii X mogą generować próby dobrane przy użyciu naszego nowego schemat dobporu respondentów. Ponieważ w próbie znajduje się 6 osób więc możliwych wyników też jest 7:   
...
gdzie X oznacza "tak, oddałabym/łbym głos na partię X", a nX oznacza "nie, oddałbym głos na inną partię lub pusty".   

Nasz nowy schemat losowania respondentów generuje aż 20\*19\*18\*17\*16\*15=27907200 różnych prób. To jest znacznie, znacznie więcej niż w przypadku badania absencji wyborczej, gdzie dobieraliśmy 2 respondantów. Tak dużej liczby prób niestety nie da się rozpisać. Możemy natomiast opisać naszą zbiorowość prób przy użyciu technik analitycznych, czyli wykonując pewne obliczenia. Zacznijmy od tego, w ilu próbach pojawi się każdy obywatel. Przypomnijmy sobie jak liczyliśmy to w przypadku schematu dobierania dwóch osób. Wówczas każdy obywatel mógł utworzyć 19 par z innymi obywatelami, a dodatkowo był dobierany jako pierwszy lub drugi respondent. W konsekwencji każdy obywatel występował w 19\*2=38 próba. A co by się działo w przypadku trzyosobowych prób? W takim przypadku dla każdego obywatela można dobrać najpierw jedną inną osobę spośród 19, a później drugą spośród pozostałych 18. Sam obywatel natomiast mógłby zajmować pierwsze, drugie lub trzecie miejsce w próbie obok dobranych do niego osób. W efekcie każdy obywatel występowałby w 19\*18\*3=1026 trójkach. Analogicznie dla sześcioosobowej próby, każdy obywatel może utworzyć szóstkę najpierw z jednym z 19, potem jednym z 18, potem jednym z 17 itd. aż wreszcie jednym z 15 obywateli. Co więcej może on zajmować 1,2,3,4,5 lub 6 miejsce w próbie. Ostatecznie więc każdy obywatel występuje w 19\*18\*17\*16\*15\*6=8372160 sześcioosobowych próbach. Potwierdzają to obliczenia programu R:
...
Zastanówmy się, ile może być takich prób, w których poparcie dla partii X wyniesie 83,3% (5/6). Przykładowo może to być próba (X, X, X, X, X, nX). Na ten moment nie będzie nas interesowało, którzy to obywatele (o jakich wartościach liczyb porządkowej). Rozróżniamy tylko czy popierają partię "X" bądź nie "nX". Losowanie tego rodzaju próby przeprowadzimy w następujący sposób. __[KONIECZNE OBJAŚNIENIE - skąd nagle losowanie spośród 7 osób]__. Pierwszego respondenta X dobieramy spośród wszystkich osób popierających partię X w populacji - lącznie jest ich 7. Potem drugiego respondenta dobieramy spośród pozostałych 6 osób popierających X, potem trzeciego respodenta dobieramy spośród pozostałych 5 osób popierających X, itd aż dochodzimy do szóstego respondenta. Dobierzemy go z innej grupy, osób niepopierających X (nX). W populacji jest ich 13. Ostatecznie więc liczba prób dla których preferencje układają się w kolejności (X, X, X, X, X, nX) jest równa 7\*6\*5\*4\*3\*13=32760. No tak, ale my chcemy wiedzieć, ile jest dokładnie wszystkich prób, w  których poparcie wynosi 83,3%, nie tylko takich, w których osoba nX ( niepopierająca X) została wylosowana jako ostatnia. Żeby się tego dowiedzieć musimy zauważyć, że obywatel deklarujący nX może zostać wylosowany, jako szósty, piąty, czwarty, … lub jako pierwszy. Czyli może on zostać ustawiony na sześć sposobów w próbie. Innymi słowy każda próba typu (X, X, X, X, X, nX) ma swoich pięć odpowiedników, w których występują ci sami obywatele, w tej samej kolejności, ale na innych miejscach. Ostatecznie więc liczba prób w których poparcie wyniesie 83,3% (5/6) wyniesie (7\*6\*5\*4\*3\*13)\*6=32760\*6=196560.    
