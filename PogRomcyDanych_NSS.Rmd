...
kable( poparcie_dla_P, format = "markdown");
```
W stosunku do badania absencji wyborczej zmienimy niewiele. Po prostu zamiast wartości -1 w kolumnie “Glosowanie” teraz będziemy analizowali występowanie wartości 1, czyli glosowanie na partię X. Będziemy również dobierać nie 2, a 6 obywateli do próby. Tak samo jak wcześniej zastosujemy jednak losowanie proste bez zwracania i technikę CAPI do realizacji wywiadów. Pytanie w kwestionariuszu będzie brzmiało: "Gdyby wybory do parlamentu (sejmu i senatu) odbywały się w najbliższą niedzielę to czy zagłosowałaby/łby Pani/an na partię X"    

Zacznijmy od sprawdzenia, jakie poparcie dla partii X mogą generować próby dobrane przy użyciu naszego nowego schemat doboru respondentów. Ponieważ w próbie znajduje się 6 osób więc możliwych jest 7 wyników:   

  * (X, X, X, X, X, X) = 100% (6/6)    
  * (X, X, X, X, X, nX) = 83% (5/6)   
  * (X, X, X, X, nX, nX) = 67% (4/6)    
  * (X, X, X, nX, nX, nX) = 50% (3/5)    
  * (X, X, nX, nX, nX, nX) = 33% (2/6)     
  * (X, nX, xX, nX, nX, nX) = 17% (1/6)  
  * (nX, nX, nX, nX, nX, nX) = 0% (0/6)  
...
Nasz nowy schemat losowania generuje aż 20\*19\*18\*17\*16\*15=27907200 różnych prób. To jest znacznie, znacznie więcej niż w przypadku badania absencji wyborczej, gdzie dobieraliśmy 2 respondantów. Tak dużej liczby prób niestety nie da się rozpisać. Możemy natomiast opisać naszą zbiorowość prób wykonując pewne obliczenia. Zacznijmy od tego, w ilu próbach pojawi się każdy obywatel. W przypadku schematu doboru dwóch respondentów każdy obywatel mógł utworzyć 19 par z innymi osobami z populacji i dodatkowo zająć pierwsze lub drugie miejsce w próbie. W konsekwencji każdy obywatel występował w 19\*2=38 próba. A co by się stało, gdybyśmy dobierali trzyosobowe próby? W tym przypadku dla każdego obywatela możnaby dobrać najpierw jedną  osobę spośród 19, a później drugą spośród pozostałych 18. Sam obywatel natomiast mógłby zajmować pierwsze, drugie lub trzecie miejsce w próbie. W efekcie każdy obywatel występowałby w 19\*18\*3=1026 trzyosobowych próbach. Analogicznie w przypadku sześcioosobowej próby najpierw do każdego obywatela można dobrać jedną z 19 osób, potem jedną z 18 pozostałych osób, potem jeszcze jedną z pozostałych 17 osób itd., aż wreszcie jedną z ostatnich 15. Oczywiście nasz obywatel może zając 1,2,3,4,5 lub 6 miejsce w próbie. Ostatecznie więc każdy obywatel występuje w 19\*18\*17\*16\*15\*6=8372160 sześcioosobowych próbach. Potwierdzają to obliczenia programu R:
